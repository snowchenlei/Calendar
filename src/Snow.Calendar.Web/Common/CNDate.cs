...
namespace Snow.Calendar.Web.Common
{
    ...
    public class ChineseCalendar
    {
        ...
        /// 获取农历整年的24节气所对应的公历日期
        /// </summary>
        /// <param name="year">需要获取的年份</param>
        /// <returns></returns>
        public static void GetChineseAllTwenty(int cnYear, int year, ref Dictionary<string, DateTime> dicTwenty)
        {
            ...
            for (int i = 1; i <= 24; i++)
            {
                num = 525948.76 * (year - 1900) + sTermInfo[i - 1];

                newDate = baseDateAndTime.AddMinutes(num);//按分钟计算
                CNDate cnDate = ChinaDate.getChinaDate(newDate);
                if (cnDate.cnIntYear != cnYear)
                { continue; }
                dicTwenty.Add(SolarTerm[i - 1], newDate);
                if (dicTwenty.Count == 24) { break; }
                if (i == 24)
                {
                    if (SolarTerm[i - 1] != "大寒")
                    {
                        GetChineseAllTwenty(year, ++year, ref dicTwenty);
                    }
                }
            }
        }
        ...
        /// <param name="year">需要获取的年份</param>
        /// <returns></returns>
        public static Dictionary<string, DateTime> GetChineseAllTwenty(int year)
        {
            DateTime baseDateAndTime = new DateTime(1900, 1, 6, 2, 5, 0); //#1/6/1900 2:05:00 AM#
            Dictionary<string, DateTime> dicTwenty = new Dictionary<string, DateTime>();
            DateTime newDate;
            double num;
            for (int i = 1; i <= 24; i++)
            {
